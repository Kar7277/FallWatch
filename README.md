...
当面试官问"这个项目的技术亮点是什么"时：

**1. 多线程架构解耦**
> 采集、检测通过环形缓冲区解耦，各自独立运行。用互斥锁和条件变量保证线程安全，避免了忙等待浪费 CPU。

**2. V4L2 mmap 零拷贝**
> 摄像头采集没有用 read() 方式，而是用 V4L2 的 mmap 内存映射，直接把内核 DMA 缓冲区映射到用户空间，避免了 CPU 拷贝。

**3. 纯 C 手写算法**
> 帧差法和色彩空间转换都是用纯 C 自己实现的，没有依赖 OpenCV。在嵌入式平台上代码更精简，也更容易交叉编译。ARM NEON 版本还做了 SIMD 加速。

**4. 分层告警**
> 检测到摔倒后，硬件层直接 GPIO 拉高 LED（最短路径），同时异步通过 AIR780E 发短信、MQTT 推送告警到云端，并保存 JPEG 证据。Qt GUI 只负责展示，不参与告警关键路径。

**5. 可扩展设计**
> 检测模块通过事件标志位和系统解耦，可以方便地替换帧差法为更强的算法（比如接入 AI NPU 推理），不影响采集和显示模块。
